...
\begin{document}
  ...
  \runningauthor{Johan Pouwelse and Martijn de Vos}
...
\begin{itemize}
	\item Lock-in: a solid reputation on one platform is locked into that platform: you cannot move your reputation. Like other forms of lock-in, this inhibits competition and encourages monopoly behaviour. Ironically, companies in the \enquote{sharing economy} do not share reputations. Users are increasingly being protected by European Union law from such data silos.\footcite{eudataprotection} The newly defined right to data portability allows individuals to obtain and reuse their personal data for their own purposes across different services.\footcite{dataportabilityeu}
...
Blockchains clearly have potential here, 
offering transparency, reducing overhead, and especially providing a single-source-of-truth.
Various reports support this opinion.\footcite{deloitteblockchain}\,\footcite{coindesklandregistration}
% RV: make a case why things like transparency are important:
The importance of transparency that blockchains offer is even more apparent 
when one considers regions in which public's trust has been damaged,
e.g., in war-torn Colombia,
and land registration requires the involvement of its citizens 
% RV: please add this to the bibliography and turn it into a footcite:
(\url{https://www.kadaster.nl/-/landrechten-voor-vrede-in-colombia}).
...
% RV: Expand a bit on how blockchains could be used for land registration:
Deeds that currently are stored in a centralized cadastral register,
  such as land purchases, mortgages and splits,
could easily be mapped onto transactions in a blockchain.
Things like inheritance rights or easements could be implemented through smart contracts % RV: Any nice reference that could be cited here?
which could rely on, e.g., the existence of attribute attestation transactions~(Section~\label{sec:trustchain}).
% RV: About query support:
In addition to recording the aforementioned transactions,
  land registration also requires its registers to be queryable and thus to be robust and highly available
  in order to provide services and certainty to citizens.
% RV: Please proofread the next few sentences and cite relevant references
Support for queries,
  such as ``what are the exact boundaries of my plot?''
  or ``what are the plots in this particular area?'',
  can be realized through a distributed search index on top of the blockchain.
An example of a real-world highly available distributed search index can be found in Tribler~(INCLUDE MOST RECENT RELEVANT TRIBLER PAPER HERE). % RV: Please add a citation here
%
...
We demonstrated the feasibility of an open-source fingerprint recognition framework, requiring minimal resources.
...
We created our decentralized marketplace and integrated it into Tribler.\footcite{pouwelse2008tribler}
